...
END {
    ...
    tot_cpu = p_tot_cpu["dedicated"] + 0;
    pd_cnt = p_pd_jobs["dedicated"] + 0;
    nodes_cnt = p_idle_nodes["dedicated"] + 0;
    if (nodes_cnt > 0) {
        res_raw = sprintf("%'"'"'d/%'"'"'d idle cores (%d fully empty nodes)", free_cpu, tot_cpu, nodes_cnt);
    } else {
        res_raw = sprintf("%'"'"'d/%'"'"'d idle cores", free_cpu, tot_cpu);
    }
    if (free_cpu == 0) {
    ...
    tot_comm = p_tot_cpu["common"] + 0;
    pd_comm = p_pd_jobs["common"] + 0;
    nodes_comm = p_idle_nodes["common"] + 0;
    if (nodes_comm > 0) {
        res_raw = sprintf("%'"'"'d/%'"'"'d idle cores (%d fully empty nodes)", free_comm, tot_comm, nodes_comm);
    } else {
        res_raw = sprintf("%'"'"'d/%'"'"'d idle cores", free_comm, tot_comm);
    }
    if (free_comm == 0) {
        res_str = sprintf("%s%s%s", C_RED, res_raw, C_RESET);
    ...
    tot_long = p_tot_cpu["long"] + 0;
    pd_long = p_pd_jobs["long"] + 0;
    nodes_long = p_idle_nodes["long"] + 0;
    if (nodes_long > 0) {
        res_raw = sprintf("%'"'"'d/%'"'"'d idle cores (%d fully empty nodes)", free_long, tot_long, nodes_long);
    } else {
        res_raw = sprintf("%'"'"'d/%'"'"'d idle cores", free_long, tot_long);
    }
    if (free_long == 0) {
        res_str = sprintf("%s%s%s", C_RED, res_raw, C_RESET);
    ...
    tot_clc = p_tot_cpu["clcgwb"] + 0;
    nodes_clc = p_idle_nodes["clcgwb"] + 0;
    if (nodes_clc > 0) {
        res_raw = sprintf("%'"'"'d/%'"'"'d idle cores (%d fully empty nodes)", free_clc, tot_clc, nodes_clc);
    } else {
        res_raw = sprintf("%'"'"'d/%'"'"'d idle cores", free_clc, tot_clc);
    }
    if (free_clc == 0) {
        res_str = sprintf("%s%s%s", C_RED, res_raw, C_RESET);
    } else {
